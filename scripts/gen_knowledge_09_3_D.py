...
"""gen_knowledge_09_3_D.py -- Generate 500 ShareGPT knowledge samples (mixed).

Subagent D of phase 09.3-01. Siblings A (science+math), B (history+arts),
C (tech+everyday+current_events) already produced batches 08-A/B/C.
This batch covers a fresh slice across all 7 categories with topics that
were not heavily exercised by A/B/C, plus all 6 sub-angles.

Seed: 1009309D (deterministic shuffle of (cat, topic, sub_angle) seeds
from the local kernel bank).

Each sample is a 3-message ShareGPT dict written to:
  datasets/knowledge/raw-09.3/batch-09-A-knowledge.jsonl
"""
import hashlib
import json
import random
...
OUT_PATH = ROOT / "datasets" / "knowledge" / "raw-09.3" / "batch-09-A-knowledge.jsonl"
SEED_STR = "1009309D"
...
# Topic kernels (loaded from companion module)
from gen_knowledge_09_3_D_data import T  # noqa: E402
...
def compose_answer(topic, angle, kernel, rng):
    ...
    if angle == "definition":
        body = f"{topic.capitalize() if not prelude else topic} is {kernel}"
        if prelude:
...
def main():
    ...
    with OUT_PATH.open("w") as f:
        for cat, topic, sa in chosen:
            ...
            a = compose_answer(topic, sa, kernel, sample_rng)
            sample = {
                "messages": [
