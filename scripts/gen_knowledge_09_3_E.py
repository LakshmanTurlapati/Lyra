...
"""gen_knowledge_09_3_E.py -- Generate 500 ShareGPT knowledge samples (mixed).

Subagent E (KNOW-B) of phase 09.3-01. Sibling generators A/B/C/D already
produced ~2000 samples. This batch covers a fresh slice across all 7
categories with topics that were not exercised by A/B/C/D, plus all 6
sub-angles.

Seed: 1009310E (deterministic shuffle of (cat, topic, sub_angle) seeds
from the local kernel bank).

Output: datasets/knowledge/raw-09.3/batch-10-A-knowledge.jsonl
"""
import hashlib
import json
import random
...
OUT_PATH = ROOT / "datasets" / "knowledge" / "raw-09.3" / "batch-10-A-knowledge.jsonl"
SEED_STR = "1009310E"
...
# Topic kernels (loaded from companion modules)
from gen_knowledge_09_3_E_data import T  # noqa: E402
...
def compose_answer(topic, angle, kernel, rng, length_bucket):
    """length_bucket in {short, medium, long} controls minimum body length."""
    ...
    if length_bucket == "short":
        # Truncate to first 1-2 sentences for short answers; do not pad.
        # Keep it concise: cap around 350 chars, do not extend.
        if len(text) > 420:
            # cut at last period before 420
            ...
            if q in seen_questions:
                for _ in range(8):
                    ...
                    if q not in seen_questions:
                        break
            # Final dedup safety: append disambiguator if still collides
            base_q = q
            dedup_i = 2
